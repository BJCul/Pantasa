...
## **1. Introduction**
Pantasa is an advanced **Tagalog grammar correction system** that integrates **rule-based** error detection with a **deep learning-based** iterative correction mechanism. The system identifies grammatical errors in Tagalog sentences by leveraging a **hybrid n-gram approach** for rule-based detection and refines sentences iteratively using a **fine-tuned deep learning model**. The goal of Pantasa is to improve the grammatical accuracy of Tagalog texts by systematically correcting errors until the sentences meet predefined linguistic rules.
