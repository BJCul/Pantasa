...
def process_pos_patterns(pos_patterns_file, generated_ngrams_file, pattern_file, output_file, model, tokenizer, threshold=0.50):
    ...
    for pattern in pos_patterns:
        ...
        if rough_pos:
            ...
            for rough_pos_match in rough_pos_matches:
                
                comparison_key = f"{rough_pos}::{rough_pos_match}"
                print(f"RoughPOS: {rough_pos}, DetailedPOS: {rough_pos_match}")
                if comparison_key not in seen_comparisons:
                    print(f"Processing new comparison: {comparison_key}")
                    _, _, comparison_matrix, new_pattern = compare_pos_sequences(rough_pos, rough_pos_match, model, tokenizer, threshold)
